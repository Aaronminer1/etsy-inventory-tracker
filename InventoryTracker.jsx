...
import { PlusCircle, Package, TrendingUp, AlertTriangle, Download, Edit2, Trash2, X, Search, BarChart3, PieChart, DollarSign, ShoppingCart, ArrowUpRight, ArrowDownRight, Calendar, Tag, MapPin, FileText, Printer, CheckSquare, Square, Upload, RefreshCw, Info, Undo2, Image, HelpCircle, ChevronRight, Shield, Cloud, Bell, BookOpen } from 'lucide-react';
import { LineChart, Line, BarChart, Bar, PieChart as RePieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import { db, dbOps, migrateFromLocalStorage } from './db';

const InventoryTracker = () => {
  const getSampleData = () => ({
    products: [
      { id: '1', sku: 'ELEC-001', name: 'Wireless Mouse', category: 'Electronics', quantity: 45, costPrice: 12.50, salePrice: 24.99, supplier: 'TechSupply Co', location: 'Warehouse A-1', reorderPoint: 15, minStock: 10, maxStock: 100, lastRestocked: '2025-10-15', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=100&h=100&fit=crop', notes: '' },
      { id: '2', sku: 'ELEC-002', name: 'USB-C Cable', category: 'Electronics', quantity: 8, costPrice: 3.25, salePrice: 9.99, supplier: 'TechSupply Co', location: 'Warehouse A-1', reorderPoint: 20, minStock: 15, maxStock: 150, lastRestocked: '2025-09-20', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1590736969955-71cc94901144?w=100&h=100&fit=crop', notes: '' },
      { id: '3', sku: 'ELEC-003', name: 'Bluetooth Speaker', category: 'Electronics', quantity: 22, costPrice: 28.00, salePrice: 59.99, supplier: 'Audio Direct', location: 'Warehouse A-2', reorderPoint: 10, minStock: 8, maxStock: 50, lastRestocked: '2025-10-25', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1608043152269-423dbba4e7e1?w=100&h=100&fit=crop', notes: '' },
      { id: '4', sku: 'CLTH-001', name: 'Cotton T-Shirt', category: 'Clothing', quantity: 0, costPrice: 8.50, salePrice: 19.99, supplier: 'FashionHub', location: 'Warehouse B-1', reorderPoint: 25, minStock: 20, maxStock: 200, lastRestocked: '2025-08-10', taxRate: 0.06, image: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=100&h=100&fit=crop', notes: '' },
      { id: '5', sku: 'CLTH-002', name: 'Denim Jeans', category: 'Clothing', quantity: 15, costPrice: 22.00, salePrice: 49.99, supplier: 'FashionHub', location: 'Warehouse B-1', reorderPoint: 12, minStock: 10, maxStock: 80, lastRestocked: '2025-10-30', taxRate: 0.06, image: 'https://images.unsplash.com/photo-1542272604-787c3835535d?w=100&h=100&fit=crop', notes: '' },
      { id: '6', sku: 'HOME-001', name: 'Ceramic Mug', category: 'Home & Kitchen', quantity: 67, costPrice: 4.75, salePrice: 12.99, supplier: 'HomeGoods Plus', location: 'Warehouse C-1', reorderPoint: 30, minStock: 25, maxStock: 200, lastRestocked: '2025-11-01', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?w=100&h=100&fit=crop', notes: 'Bestseller' },
      { id: '7', sku: 'HOME-002', name: 'Cutting Board', category: 'Home & Kitchen', quantity: 5, costPrice: 15.00, salePrice: 34.99, supplier: 'HomeGoods Plus', location: 'Warehouse C-1', reorderPoint: 15, minStock: 10, maxStock: 60, lastRestocked: '2025-09-15', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1594735153628-b67f9c60f7cc?w=100&h=100&fit=crop', notes: '' },
      { id: '8', sku: 'HOME-003', name: 'Glass Storage Set', category: 'Home & Kitchen', quantity: 31, costPrice: 18.50, salePrice: 39.99, supplier: 'KitchenPro', location: 'Warehouse C-2', reorderPoint: 20, minStock: 15, maxStock: 100, lastRestocked: '2025-10-20', taxRate: 0.08, image: 'https://images.unsplash.com/photo-1603399587513-136aa9398f2d?w=100&h=100&fit=crop', notes: '' }
    ],
    movements: [
  ...
  // Initialize - migrate from localStorage if needed, then load from IndexedDB
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        if (migrationResult.success && !migrationResult.alreadyMigrated) {
          console.log(`Migrated ${migrationResult.productsCount} products to IndexedDB`);
        }
  ...
  // Save to IndexedDB when products change (NOT in demo mode)
  useEffect(() => {
                              ...
                              <td colSpan="10" className="px-6 py-6">
                                <div className="flex gap-6">
                                  ...
                                  <div className="flex-1 grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                      <h3 className="text-sm font-semibold text-slate-400 mb-1">Stock Levels</h3>
                                      <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                        ...
                                        <div className="flex justify-between">
                                          ...
                                          <span className="text-white">{p.minStock}</span>
                                        </div>
                                        <div className="flex justify-between">
                                          <span className="text-slate-400">Max Stock:</span>
                                          <span className="text-white">{p.maxStock}</span>
                                        </div>
                                      </div>
                                    </div>
                                    {p.notes && (
                                      <div className="col-span-2">
                                        ...
                                        <p className="text-white text-sm bg-slate-800 rounded p-3">{p.notes}</p>
                                      </div>
                                    )}
                                  </div>
                                </div>
